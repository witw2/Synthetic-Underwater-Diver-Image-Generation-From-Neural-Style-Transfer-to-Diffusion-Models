...
def preprocess_image(image, target_size, remove_bg=False, should_save=False):
    ...
    # Apply the usual preprocessing transformations
    ...
    if should_save:
        ...
        pil_image = transforms.ToPILImage()(image.squeeze(0).div(255))
        ...
        preprocessed_dir = os.path.join(project_dir, '..', 'project','data', 'preprocessed')
...
def postprocess_image(tensor):
    image = tensor.squeeze(0).clamp(0, 255).div(255).cpu()
    transform = transforms.ToPILImage()
    image = transform(image)
    return image

# Function to combine styles
def combine_styles(style_image_paths, size=(400, 400)):
    # Load all style images and resize them
    style_images = [Image.open(path).resize(size) for path in style_image_paths]

    # Convert images to numpy arrays
    style_arrays = [np.array(img).astype(np.float32) for img in style_images]

    # Compute the average of the style arrays
    avg_style_array = np.mean(style_arrays, axis=0)

    # Convert the averaged array back to an image
    avg_style_image = Image.fromarray(np.uint8(avg_style_array))

    return avg_style_image

# Function to read a dictionary from a file
def read_dict_from_file():
    script_dir = os.path.dirname(os.path.abspath(__file__))
    file_path = os.path.join(script_dir, 'params.txt')
